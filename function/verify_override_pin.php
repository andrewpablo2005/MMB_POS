...
try {
    $stmt = $db->prepare("SELECT u.id, u.username, u.password, u.position, ui.firstname, ui.lastname
                          FROM users u LEFT JOIN users_info ui ON ui.user_id = u.id
                          WHERE u.username = ? AND u.status = 'active'");
    $stmt->execute([$username]);
    $user = $stmt->fetch(PDO::FETCH_ASSOC);
    $valid = $user && password_verify($password, $user['password'])
        && in_array(strtolower((string)$user['position']), ['owner', 'admin'], true);
    if (!$valid) {
    ...
    echo json_encode(['success' => true, 'approver_id' => (int)$user['id'], 'approver_name' => $name]);
} catch (Throwable $e) {
    error_log('verify_override_pin error: ' . $e->getMessage());
    echo json_encode(['error' => 'Unable to verify override authorization']);
}
